    ...
    public void saveAppointment(ActionEvent event) throws IOException, SQLException {
        ...
        for (int i = 0; i < Appointments.getAllAppointments().size(); i++) {
            if (Appointments.getAllAppointments().get(i).getStart().substring(0, 10).equals(assignedDate.getValue().toString())
                    ...
                    || Timestamp.valueOf(start).after(Timestamp.valueOf(Appointments.getAllAppointments().get(i).getStart())) && Timestamp.valueOf(start).before(Timestamp.valueOf(Appointments.getAllAppointments().get(i).getEnd()))
                    || Timestamp.valueOf(start).equals(Timestamp.valueOf(Appointments.getAllAppointments().get(i).getStart()))
                    || Timestamp.valueOf(start).before(Timestamp.valueOf(Appointments.getAllAppointments().get(i).getStart())) && Timestamp.valueOf(end).after(Timestamp.valueOf(Appointments.getAllAppointments().get(i).getStart())))) {
                valid = false;
